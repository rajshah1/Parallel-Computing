...
void countWords(MyHashtable<std::string, int> &ht, std::vector<std::string> filecontent){

  // std::cout<<"size : "<<filecontent.size()<<std::endl;
  for (int i=0; i<filecontent.size(); i++) {
      // mut.lock();
      ht.update_increment(filecontent[i]);
      // mut.unlock();
  }
}
...
int main(int argc, char **argv)
{
  ...
  for(auto & filecontent:wordmap){
    threadVec.push_back(std::thread(countWords, std::ref(ht), std::ref(filecontent)));
  }
